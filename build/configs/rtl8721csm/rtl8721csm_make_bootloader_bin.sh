...
#ARCHSCRIPT = -T$(TOPDIR)/../build/configs/$(CONFIG_ARCH_BOARD)/scripts/$(LDBOOTSCRIPT)

#LDFLAGS += $(ARCHSCRIPT)

#arm-none-eabi-ld $(LDFLAGS) target_loader.axf $(BOOTOBJS) 

#cp $BINDIR/tinyara.axf $BINDIR/target_loader.axf

arm-none-eabi-nm $BINDIR/target_loader.axf | sort > $BINDIR/target_loader.map

arm-none-eabi-objdump -d $BINDIR/target_loader.axf > $BINDIR/target_loader.asm

cp $BINDIR/target_loader.axf $BINDIR/target_pure_loader.axf

arm-none-eabi-strip $BINDIR/target_pure_loader.axf

arm-none-eabi-objcopy -j .ram_image1.entry -j .ram_image1.text -j .ram_image1.data \
-Obinary $BINDIR/target_pure_loader.axf $BINDIR/ram_1.bin

arm-none-eabi-objcopy -j .xip_image1.text \
-Obinary $BINDIR/target_pure_loader.axf $BINDIR/xip_boot.bin

arm-none-eabi-size -A --radix=16 $BINDIR/target_loader.axf

arm-none-eabi-size -t --radix=16 $BINDIR/target_loader.axf

echo "========== Image manipulating start =========="
$GNUUTL/prepend_header.sh $BINDIR/ram_1.bin  __ram_start_table_start__  $BINDIR/target_loader.map
$GNUUTL/prepend_header.sh $BINDIR/xip_boot.bin  __flash_boot_text_start__  $BINDIR/target_loader.map
python $GNUUTL/code_analyze.py

cat $BINDIR/xip_boot_prepend.bin $BINDIR/ram_1_prepend.bin > $BINDIR/km0_boot_all.bin
